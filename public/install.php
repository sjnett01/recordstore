...
function installer_config(array $data,string $privateRoot,string $publicRoot): string {
    $config=['app'=>['base_url'=>rtrim($data['base_url'],'/'),'timezone'=>$data['timezone'],'worker_secret'=>bin2hex(random_bytes(32)),'download_limit_default'=>3,'demo_checkout'=>false,'ffmpeg_path'=>'/usr/bin/ffmpeg','ffprobe_path'=>'/usr/bin/ffprobe','master_upload_max_bytes'=>536870912],'payments'=>['currency'=>'GBP','paypal'=>['sandbox'=>['client_id'=>'REPLACE_WITH_PAYPAL_SANDBOX_CLIENT_ID','client_secret'=>'REPLACE_WITH_PAYPAL_SANDBOX_CLIENT_SECRET','webhook_id'=>'REPLACE_WITH_PAYPAL_SANDBOX_WEBHOOK_ID'],'live'=>['client_id'=>'REPLACE_WITH_PAYPAL_LIVE_CLIENT_ID','client_secret'=>'REPLACE_WITH_PAYPAL_LIVE_CLIENT_SECRET','webhook_id'=>'REPLACE_WITH_PAYPAL_LIVE_WEBHOOK_ID']]],'db'=>['host'=>$data['db_host'],'port'=>(int)$data['db_port'],'name'=>$data['db_name'],'user'=>$data['db_user'],'pass'=>$data['db_pass'],'charset'=>'utf8mb4'],'paths'=>['masters'=>$privateRoot.'/masters','previews'=>$publicRoot.'/previews','artwork'=>$publicRoot.'/artwork']];
...
elseif($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        ...
        if(!in_array($data['scheduler'],['cron','plesk','webcron','lazy'],true))throw new RuntimeException('Choose a valid scheduler method.');

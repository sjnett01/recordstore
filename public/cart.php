...
<?php if ($tracks): ?>
  ...
  <div class="cart-summary panel"><div class="cart-pricing"><span>Subtotal</span><strong><?=money($total)?></strong><?php if($discount['ok']&&$discount['discount_pence']>0):?><span>Discount (<?=e($discount['code'])?>)</span><strong class="discount-value">−<?=money((int)$discount['discount_pence'])?></strong><?php endif;?><span>Total</span><strong><?=money((int)$discount['total_pence'])?></strong></div><?php $pm=paypal_mode(); ?><form method="post" action="checkout" class="checkout-start-form"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><?php if($viewer):?><label class="check checkout-newsletter-optin"><input type="checkbox" name="newsletter_opt_in" value="1"> <span>Do you want to be notified when new tracks are released? Check this box.</span></label><?php endif;?><button class="button" type="submit">Pay with PayPal<?php if($pm==='sandbox'):?> · TEST<?php endif;?></button></form></div>

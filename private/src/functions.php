...
function track_artist_links(array $track): string { static $cache=[]; $id=(int)($track['id']??$track['track_id']??0); if($id<=0)return e((string)($track['artist_name']??'')); if(!isset($cache[$id])){try{$st=$GLOBALS['pdo']->prepare("SELECT a.name,a.slug FROM track_artists ta JOIN artists a ON a.id=ta.artist_id WHERE ta.track_id=? ORDER BY ta.sort_order,ta.artist_id");$st->execute([$id]);$cache[$id]=$st->fetchAll();}catch(Throwable $ignored){$cache[$id]=[];}} if(!$cache[$id])return e(track_artist_credit($track)); $links=[]; foreach($cache[$id] as $artist){$links[]='<a href="'.e(url('artists/'.rawurlencode(artist_public_slug($artist)))).'">'.e((string)$artist['name']).'</a>';} return implode(' <span class="artist-credit-separator">x</span> ',$links); }
...
function layout_footer(): void { ?>
    ...
    <script src="<?=e(asset_url('assets/js/app.js?v=1.13.62'))?>"></script><script src="<?=e(asset_url('assets/js/player.js?v=1.13.62'))?>"></script></body></html><?php }
